...
# def check_all_color2(color_dict):
#     '''
#     Checks all colors in a chart to ensure that color
#     combinations are accessible
...
#     '''
#     issues = {}
#     # Initialize variables w/ nulls
#     var_dict = {}
#     # null_list = [x = None for x in var_list]

#     for key in color_dict.keys():
#         if key == 'config':
#             for k in color_dict[key].keys():
#                 if k == 'range':
#                     palette_iss = check_palette(color_dict[key][k])
#                     if palette_iss:
#                         issues['palette'] = palette_iss
#                 if k == 'background':
#                     background = color_dict[key][k]
#                     var_dict['background'] = background
#         elif key == 'text':
#             for k in color_dict[key].keys():
#                 if k == 'color':
#                     txt_color = color_dict[key][k]
#                     var_dict['text_color'] = txt_color
#         elif key == 'title':
#             for k in color_dict[key].keys():
#                 if k == 'color':
#                     title_color = color_dict[key][k]
#                     var_dict['title_color'] = title_color
#         elif key == 'encoding':
#             for k in color_dict[key].keys():
#                 if k == 'color':
#                     for key, val in color_dict[key][k].items():
#                         if key == 'value':
#                             spec_filled = val
#                             var_dict['fill_color'] = spec_filled
#                         if key == 'condition':
#                             check_col = val['value']
#                             var_dict['conditional_fill'] = check_col
#         elif key == 'axisX':
#             for k in color_dict[key].keys():
#                 if k == 'color':
#                     x_axis_col = color_dict[key][k]
#                     var_dict['x_axis_color'] = x_axis_col
#         elif key == 'axisY':
#             for k in color_dict[key].keys():
#                 if k == 'color':
#                     y_axis_col = color_dict[key][val]
#                     var_dict['y_axis_color'] = y_axis_col

#     background_col = var_dict['background']
#     for key in var_dict.keys():
#         if key != 'background':
#             check_and_fill((background_col, var_dict[key]), issues,
#                 f"{key} to background", 'text')
#     return issues
